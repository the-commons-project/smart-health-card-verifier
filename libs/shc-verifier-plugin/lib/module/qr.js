...
function shcChunksToJws(shc) {
  ...
  for (let shcChunk of shc) {
    ...
    if (chunkResult.result.length > MAX_QR_CHUNK_LENGTH) {
      console.log(`QR chunk ${chunkIndex} is larger than ${MAX_QR_CHUNK_LENGTH} bytes`, ErrorCode.INVALID_NUMERIC_QR);
    }

    if (jwsChunks[chunkIndex - 1]) {
      // we have a chunk index collision
      // eslint-disable-next-line @typescript-eslint/restrict-plus-operands
      console.log(`we have two chunks with index ${chunkIndex}`, ErrorCode.INVALID_NUMERIC_QR_HEADER);
      return undefined;
    } else {
  ...
  if (chunkCount > 1 && jws.length <= MAX_QR_CHUNK_LENGTH) {
    console.log(`JWS of size ${jws.length} (<= ${MAX_QR_CHUNK_LENGTH}) didn't need to be split in ${chunkCount} chunks`, ErrorCode.INVALID_QR);
  ...
  if (jwsChunks.map(jwsChunk => jwsChunk.length).reduce((unbalanced, length) => unbalanced || length < expectedChunkSize - balancedSizeBuffer || length > expectedChunkSize + balancedSizeBuffer, false)) {
    console.log('QR chunk sizes are unbalanced: ' + jwsChunks.map(jwsChunk => jwsChunk.length.toString()).join(), ErrorCode.UNBALANCED_QR_CHUNKS);
  }

  return jws;
}

function shcToJws(shc) {
  let chunkCount = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 1;
  let chunked = chunkCount > 1;
  const qrHeader = 'shc:/';
  const positiveIntRegExp = '[1-9][0-9]*';
  let chunkIndex = 1; // check numeric QR header
  ...
  if (chunked && !isChunkedHeader) {
    ...
    const found = shc.match(new RegExp(`^${qrHeader}${positiveIntRegExp}/(?<expectedChunkCount2>[1-9][0-9]*)/.*$`));
    if (found) console.log(`${found}`);

    if (hasBadChunkCount) {
      const expectedChunkCount = parseInt(shc.substring(7, 8));
      console.log(`Missing QR code chunk: received ${chunkCount}, expected ${expectedChunkCount}`, ErrorCode.MISSING_QR_CHUNK);
      return undefined;
    }
  }

  if (chunked && isChunkedHeader) {
    console.log(`Single-chunk numeric QR code should have a header ${qrHeader}, not ${qrHeader}1/1/`, ErrorCode.INVALID_NUMERIC_QR_HEADER);
  ...
  if (!validQrHeader) {
    ...
    console.log(`Invalid numeric QR header: expected ${expectedHeader}`, ErrorCode.INVALID_NUMERIC_QR_HEADER);
    return undefined;
  ...
  if (Math.max(...digitPairs.map(d => Number.parseInt(d))) > 77) {
    console.log("Invalid numeric QR code, one digit pair is bigger than the max value 77 (encoding of 'z')." + 'Make sure you followed the encoding rules.', ErrorCode.INVALID_NUMERIC_QR);
    return undefined;
  } // breaks string array of digit pairs into array of numbers: 'shc:/123456...' = [12,34,56,...]


  const jws = digitPairs // for each number in array, add an offset and convert to a char in the base64 range
  .map(c => String.fromCharCode(Number.parseInt(c) + b64Offset)) // merge the array into a single base64 string
  .join('');
  return {
    result: jws,
