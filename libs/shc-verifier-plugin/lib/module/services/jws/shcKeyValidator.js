...
const addIfKeyIsValidSHCFormat = async key => {
  if (!key.kid || typeof key.kid !== 'string') {
    console.log('key does not contain kid field', ErrorCode.INVALID_KEY_PRIVATE);
    return false;
  }
  ...
  if (!key.alg) {
    ...
    console.log(`${keyName}: wrong algorithm in issuer key. expected: 'ES256', actual: ${String(key.alg)}`, ErrorCode.INVALID_KEY_WRONG_ALG);
    return false;
  ...
  if (!key.kty) {
    ...
    console.log(`${keyName}:wrong key type in issuer key. expected: 'EC', actual: ${String(key.kty)} `, ErrorCode.INVALID_KEY_WRONG_KTY);
    return false;
  ...
  if (!key.use) {
    ...
    console.log(`${keyName}:wrong usage in issuer key. expected: 'sig', actual: ${String(key.use)}`, ErrorCode.INVALID_KEY_WRONG_USE);
    return false;
  }
  ...
  try {
    ...
    if (key.kid !== thumbprint) {
      console.log(`${keyName}:'kid' does not match thumbprint in issuer key. expected: \
                    ${String(thumbprint)} , actual: ${String(key.kid)}`, ErrorCode.INVALID_KEY_WRONG_KID);
      jwkKey.error = new InvalidError(ErrorCode.INVALID_KEY_WRONG_KID);
      return false;
    }
  } catch (err) {
    console.log(`${keyName}: Failed to calculate issuer key thumbprint : ${String(err.message)}`, ErrorCode.INVALID_KEY_UNKNOWN);
    return false;
  }
...
export const verifyAndImportHealthCardIssuerKey = async keySet => {
  ...
  for (let i = 0; i < keySet.keys.length; i++) {
    ...
    try {
      ...
      console.log('Error adding key to keyStore : ' + error.message, ErrorCode.INVALID_KEY_UNKNOWN);
    }
  }
};
