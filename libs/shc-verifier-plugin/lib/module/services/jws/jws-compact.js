...
export async function validate(jws) {
  ...
  if (jws.length > MAX_JWS_SINGLE_CHUNK_LENGTH) {
    console.log(`JWS is longer than ${MAX_JWS_SINGLE_CHUNK_LENGTH} characters, and will result in split QR codes`, ErrorCode.JWS_TOO_LONG);
  }
  ...
  if (!isJws) {
    console.log("Failed to parse JWS-compact data as 'base64url.base64url.base64url' string.", ErrorCode.JSON_PARSE_ERROR);
    return false;
  } // failures will be recorded in the console.logan continue processing.


  validateSchema(jwsCompactSchema, jws); // split jws into header, payload, key
  ...
  const payload = Utils.parseJson(inflatedPayload || b64DecodedPayloadString || rawPayload);
...
async function downloadAndImportValidKeys(issuerURL) {
  ...
  try {
    ...
    try {
      ...
      console.log(`Can't parse downloaded issuer JWK set: ${String(err.message)}`, ErrorCode.ISSUER_KEY_DOWNLOAD_ERROR);
      return false;
    }
  } catch (err) {
    if (err instanceof InvalidError) throw err;
    console.log(`Failed to download issuer JWK set:  ${String(err.message)}`, ErrorCode.ISSUER_KEY_DOWNLOAD_ERROR); // return undefined
...
function checkJwsHeader(header) {
  ...
  if (headerBytes) {
    headerJson = Utils.parseJson(headerBytes.toString());
    ...
    if (headerJson == null) {
      ...
      if (!headerKeys.includes('alg')) {
        ...
        console.log(`Wrong value for JWS header property 'alg' property expected: "ES256", actual: "${headerJson.alg}".`, ErrorCode.JWS_HEADER_ERROR);
      }
      ...
      if (!headerKeys.includes('zip')) {
        ...
        console.log(`Wrong value for JWS header property 'zip' property expected: "DEF", actual: "${headerJson.zip}".`, ErrorCode.JWS_HEADER_ERROR);
      }
...
function checkJwsSignatureFormat(key) {
  ...
  if (sigBytes && sigBytes.length > 64 && sigBytes[0] === 0x30 && sigBytes[2] === 0x02) {
    console.log('Signature appears to be in DER encoded form. Signature is expected to be 64-byte r||s concatenated form.\n' + 'See https://tools.ietf.org/html/rfc7515#appendix-A.3 for expected ES256 signature form.', ErrorCode.SIGNATURE_FORMAT_ERROR); // DER encoded signature will constructed as follows:
    ...
    console.log('Signature is ' + sigBytes.length.toString() + '-bytes. Signature is expected to be 64-bytes', ErrorCode.SIGNATURE_FORMAT_ERROR);
  }
...
function checkJwsPayload(rawPayload) {
  ...
  if (b64DecodedPayloadBuffer) {
    try {
      inflatedPayload = pako.inflateRaw(b64DecodedPayloadBuffer, {
        to: 'string'
      }).trim();
    } catch (err) {
      // try normal inflate
      try {
        inflatedPayload = pako.inflate(b64DecodedPayloadBuffer, {
          to: 'string'
        }).trim();
        console.log('Error inflating JWS payload. Compression should use raw DEFLATE (without wrapper header and adler32 crc)', ErrorCode.INFLATION_ERROR);
      } catch (err) {
        console.log(['Error inflating JWS payload. Did you use raw DEFLATE compression?', err].join('\n'), ErrorCode.INFLATION_ERROR); // inflating failed, let's try to parse the base64-decoded string directly
...
async function verifyJws(jws, kid) {
  ...
  const verifier = jose.JWS.createVerify(KeysStore.store);
